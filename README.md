...
- `/knockback [amount] [target]` - Put knockback on anything! can be 100 or whatever. Default level: 10. Optionally target a player's held item.
- `/knockbackstick [amount] [target]` - Similar to the command above, give yourself or a target player a stick with knockback. Default level: 10.

#### Player
- `/heal <players> [alsoFeed true/false]` - Heal specified players to max health! Also feeds them if true
- `/feed [players]` - Feeds specified players, or yourself if none specified!
...
- `/explode <x y z> <power> [createFire true/false]` - Explodes at a set position with specified explosion power and creates fire on explosion!
...
#### Gamemode
- `/survival [players]` - Sets the gamemode to survival of specified players, or yourself if none specified!
- `/creative [players]` - Same as above
- `/adventure [players]` - Same as above
- `/spectator [players]` - Same as above
